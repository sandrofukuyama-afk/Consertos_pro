  ...
  return (
    <form action={closeDiagnosticAction} className="mt-5 grid gap-3">
      ...
      >
        ...
        <option value="probable">Provavel</option>
      ...
      >
        ...
        <option value="liquid_damage">Dano por liquido</option>
        <option value="thermal_failure">Falha termica</option>
      ...
      <textarea
        ...
        rows={3}
        placeholder="Explicacao tecnica da causa"
        className="rounded-2xl border border-[var(--panel-border)] bg-[var(--background)] px-4 py-3 text-sm outline-none"
      />
      <select
        name="solution_type"
        ...
        className="rounded-2xl border border-[var(--panel-border)] bg-[var(--background)] px-4 py-3 text-sm outline-none"
      >
        <option value="">Tipo de solucao</option>
        <option value="component_replacement">Troca de componente</option>
        <option value="rework">Retrabalho</option>
        <option value="firmware_flash">Regravacao</option>
        <option value="jumper">Jumper</option>
        <option value="cleaning">Limpeza</option>
        <option value="reballing">Reballing</option>
        <option value="configuration_change">Ajuste de configuracao</option>
        <option value="other">Outra</option>
      </select>
      <input
        type="text"
        name="solution_title"
        placeholder="Titulo da solucao"
        className="rounded-2xl border border-[var(--panel-border)] bg-[var(--background)] px-4 py-3 text-sm outline-none"
      />
      <textarea
