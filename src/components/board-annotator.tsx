...
export function BoardAnnotator({
  ...
  const [tempPin, setTempPin] = useState<{
    ...
    y: number;
    note: string;
    color: ComponentAnnotation["color"];
  } | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-y-auto rounded-[28px] border border-[var(--panel-border)] bg-[var(--panel)] p-6 text-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div>
            ...
            <p className="text-sm text-[rgba(230,228,245,0.6)]">
              Clique em qualquer ponto da placa na imagem para inserir uma anotacao tecnica.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {errorMessage ? (
          <div className="mt-4 rounded-xl border border-red-500/30 bg-red-950/50 p-3 text-sm text-red-300">
            {errorMessage}
          </div>
        ) : null}
            ...
            >
              ...
              {pins.map((pin) => {
                ...
                return (
                  <div
                    key={pin.id}
                    className="group absolute flex h-5 w-5 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full text-[10px] font-bold text-white shadow-[0_0_8px] transition-transform hover:scale-125"
                    style={{ left: `${pin.x}%`, top: `${pin.y}%` }}
                    title={pin.note}
                  >
                    <span className={`h-3.5 w-3.5 rounded-full border border-white ${colorBg}`} />
                    <div className="absolute bottom-6 left-1/2 z-10 hidden w-48 -translate-x-1/2 rounded-lg border border-white/10 bg-black/90 p-2 text-center text-xs leading-normal text-white group-hover:block">
                      {pin.note}
                    </div>
                  </div>
                );
          ...
          <div className="flex flex-col gap-4">
            {tempPin ? (
              <div className="rounded-[22px] border border-[var(--accent-copper)]/30 bg-[rgba(109,94,242,0.08)] p-4">
                <h4 className="text-sm font-bold text-[var(--accent-copper)]">Novo Ponto Tecnico</h4>
                <div className="mt-3">
                  <label className="text-xs text-[rgba(230,228,245,0.6)]">Nota do problema</label>
                  <input
                    type="text"
                    value={tempPin.note}
                    onChange={(event) => setTempPin({ ...tempPin, note: event.target.value })}
                ...
                <div className="mt-3">
                  <label className="text-xs text-[rgba(230,228,245,0.6)]">Cor da marcacao</label>
                  <select
                    value={tempPin.color}
                    onChange={(event) =>
                      setTempPin({
                        ...tempPin,
                        color: event.target.value as ComponentAnnotation["color"],
                      })
                    }
                    className="mt-1 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-sm text-white outline-none focus:border-[var(--accent-copper)]"
                  >
                    <option value="red">Vermelho (Falha / Aquecimento)</option>
                    <option value="yellow">Amarelo (Atencao / Suspeito)</option>
              ...
              <div className="rounded-[22px] border border-white/5 bg-white/5 p-4 text-center">
                <p className="text-sm text-[rgba(230,228,245,0.7)]">
                  Clique na placa para registrar um ponto de atencao.
                </p>
              </div>
            )}

            <div className="flex-1">
              <h4 className="border-b border-white/10 pb-2 text-sm font-semibold">
              ...
              <ul className="mt-3 max-h-[30vh] space-y-2 overflow-y-auto pr-1">
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="px-2 font-semibold text-red-400 hover:text-red-300"
                      >
                        Remover
                      </button>
                    </li>
                ...
                {pins.length === 0 ? (
                  <p className="mt-4 text-center text-xs text-[rgba(230,228,245,0.4)]">
        ...
        <div className="mt-6 flex justify-end gap-3 border-t border-white/10 pt-4">
          <button
            onClick={onClose}
            disabled={isSaving}
            className="rounded-full bg-white/10 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-white/20 disabled:opacity-50"
          >
            Fechar sem salvar
          </button>
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex items-center gap-2 rounded-full bg-[var(--accent-copper)] px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[rgba(109,94,242,0.2)] transition hover:opacity-90 disabled:opacity-50"
          >
            {isSaving ? (
              <>
                <svg
